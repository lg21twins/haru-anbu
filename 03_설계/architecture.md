...
**v1.0 | 2026.03 (2026-04-19 갱신 노트 추가)**

---

## 📌 2026-04-19 갱신 노트 — 클라이언트 4트랙 분리

v1.0 원문은 "클라이언트 = 보호자/환자/의료진 3역할 단일 SPA" 구조를 전제한다. 이후 기획 진화로 **클라이언트가 실제 4개 트랙으로 분리**되어 구현 중이므로, 본문 §1 클라이언트 계층은 아래 최신 사실로 대체해서 읽어야 한다.

**현행 4트랙 (2026-04-19 기준)**

| 트랙 | 폴더 | 단말 | 구현 형태 | 특징 |
|---|---|---|---|---|
| 보호자 앱 | `v9.5_보호자앱/` | 모바일 (PWA) | 단일 HTML SPA (`g-guardian-live.html` 외) | 5탭 14화면, glassmorphism, SOS 글로벌 배너, AI 플로팅 |
| 의료진 웹 | `v10_의료진웹/` | PC 데스크톱 | 단일 파일 SPA (`m.html`) | 240px 사이드바 8메뉴, 탭별 시각 메타포 5종 |
| 요양보호사 앱 | `v11_요양보호사앱/` | 모바일 (PWA) | 4개 HTML (c01~c04) + 플로팅 SOS | 시설 리서치 6축, 8단계 스텝퍼, 법정 6분류 |
| 환자 태블릿 | `v12_환자앱/` | 태블릿 10~11" 거치형 | 10개 HTML (P-01~P-10, P-04 제외) | 멀티 페이지, 18px 본문, 카드 한 개 = 한 화면 |

**원문과 달라진 점**
- 환자 태블릿은 "단일화면 AI 말벗" 컨셉을 2026-04-19 폐기하고 **P-01~P-10 멀티 페이지**로 전환.
- 의료진 계층을 "케어팀(의사·간호사·요양보호사 3역할)" 단일 카테고리로 통합 — 단, 단말 구분(PC 웹 vs 모바일)은 유지.
- 각 트랙은 **공통 토큰(`07_디자인/tokens/tokens.css`)** 과 **role-별 data-role 변형**을 공유. 토큰 SSOT 원칙은 v1.0에서 변경 없음.

하위 §2 기술 스택(React·TS·Tailwind·Supabase·Claude API·FCM)과 §3~§7 백엔드/인증/실시간/AI/배포 섹션은 v6.2 기획과 **정합 유지**. v2.0 풀 리라이트는 앱별 라우팅·번들 분리 결정 후 별도 PR로 진행 예정.
